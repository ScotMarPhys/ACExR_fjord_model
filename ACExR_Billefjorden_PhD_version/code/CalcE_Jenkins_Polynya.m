...
E.Qf = Bdata.Qf;
% E.Qf = zeros(Ndays,1);
% random number in a mormal distribution
% a = 0;
% b = 50;
% E.Qice = (b-a).*rand(Ndays,1) + a;
% % E.Qice = E.Qice.*10;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if Param.hotstart == 1
    disp(['Hot-start: loading data from '])
    hotstart =('hotstart_1.mat');
    load(hotstart);
end
...
%     rho0 = Const.rho0 - alpha * (T0 - 10) + beta * (S0 - 35);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % test the possibility of geostrophic shutdown
%     if  day >= 2 && Param.I(day-1,:) >0;
%     T0 = D.T(:,1:2);
%     S0 = D.S(:,1:2);
%     rho0 = Const.rho0 - alpha * (T0 - 10) + beta * (S0 - 35);
%     end
...
%     E.Qt(day,1) = 0 * eps * Bdata.a0(day) * Param.Af / Const.Tperiod;
    ...
    for istep = 1:Nsteps_per_day
       ...
       % reduce the heat flux if ice is grown the previous day
        if  day >= 2 && Param.I(day-1,:) > 0;
            D.Qshf = D.Qshf ;
        end
        
         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Update Volume flux. Ice growth per day (m) times cover constant
        % times area (m2) divided by time sec/day. 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            D.I =0;
            D.Qice = 0;
            
        if D.T(1)<= -1.8  ; 
            Inew            = IceGrowth(-1.8, D.T(1),D.H(1)); 
            Ice             = Inew;
            D.I             = Ice;
            D.T(1)          = -1.8; 
            D.Qice  = (Ice * (LochData.HWarea) ) / Const.deltaT ; 
...
%             
%         if 120> day >0 ||  ; 
%             Inew            = Bdata.Qshf(day)/(Const.RhoIce*Const.LIce)
%             Inew            = Inew*-1;
%             D.I             = (Inew/86400) * Const.deltaT;
%             D.T(1)          = -1.8; 
%             D.Qice          = (Inew * (197000000) ) / Const.deltaT ; 
%             
%         else
%             D.I =0;
%             D.Qice = 0;
%         end
        ...
        % reduce the wind flux if ice is grown the previous day
        if  day >= 2 && Param.I(day-1,:) >0;
            D.Qh = D.Qh ;
        end
...
%             KZ = Kz23_general;
        ...
        Tnew = TracerSI(D.T,D.T(1),T0,D.T(1));
        if Param.SHFlux == 1; 
            Hbar = 0.5 * (D.Hp(1) + D.H(1));
            Tnew(1) = Tnew(1) + Const.deltaT * D.Qshf / ...
                (Const.cp * D.rho(1) * Hbar);
        end
        [Snew] = TracerSI(D.S,0,S0,15);
        
        % Attempt to catch errors
        if Tnew(1) < -10 | Tnew(1) > 30
            Param.Error = 1;
            return;
        end
        if Tnew(2) < -10 | Tnew(2) > 30
            Param.Error = 2;
            return;
        end
...
%         D.rho = Const.rho0 - alpha * (D.T - 10) + beta * (D.S - 35);
        ...
        Param.A(day,:) = Param.A(day,:) + D.xarea / Nsteps_per_day;
...
%                         + beta * (Param.S(day,:) - 35);
    ...
    Param.rho(day,:) = sw_dens0(Param.S(day,:),Param.T(day,:));               
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Calculate daily mean fluxes and velocities
    E.Qice(day) = E.Qice(day) / Nsteps_per_day;
