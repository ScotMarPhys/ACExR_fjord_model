...
% Phil Gillibrand
% 14/3/2006
% Adjustment to the defining paramters of S and T during the
% initialiatsion, the boundary data is just repeat values beyond 90 metres
% or so. So i make the temp and salinity o flayer 3 equal to the depth of
% layer 2. see line 202 and 203 for the updated code
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Const.cp = 4200;
% Density of sea ice [Lepperanta, 1993 and Steele et al. 2008]
Const.RhoIce = 900;
% Latent heat of fusion for sea ice [Holland&Jenkins 1998]
Const.LIce = 334000;
% Effective heat transfer coeffecient [Eicken: Sea Ice textbook, 2010]
Const.k = 45;
% Salinity of sea ice
Const.IceSalinity = 10;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Define N and Af
Param.N = 2*Const.ce * (Bdata.Uw .^ 3);% entrainment constant (tunable) * wind^3
Param.Af = Hypso.A(1);
...
if Param.ICswitch == 0
    ...
    Param.A = zeros(ndays,2);
    Param.I = zeros(ndays,1);
    Param.IV = zeros(ndays,1);
    Param.IC = zeros(ndays,1);
    ...
    if LochData.Nsill == 0
        ...
        Param.S(1,1) = mean(Bdata.S_ext(1:iz1,1));
...
%         Param.S(1,3) = mean(Bdata.S_ext(iz2:iz3,1));
        Param.S(1,3) = mean(Bdata.S_ext(iz2,1));
        % Temperature
        Param.T(1,1) = mean(Bdata.T_ext(1:iz1,1));
        Param.T(1,2) = mean(Bdata.T_ext(iz1:iz2,1));
%         Param.T(1,3) = mean(Bdata.T_ext(iz2:iz3,1));                
        Param.T(1,3) = mean(Bdata.T_ext(iz2,1));
        % Tracer 1
        if Param.Ntracer > 0
            Param.C1(1,1) = mean(Bdata.C1_ext(1:iz1,1));
            Param.C1(1,2) = mean(Bdata.C1_ext(iz1:iz2,1));
...
Param.F = FreezeTemp(Param.S(1),Param.T(1));
